...
**Что сделано:**
- Убраны generic Ansible команды (заменены на конкретные Make команды)
- Сокращен размер: 124 → 86 строк (-30%)
- Добавлены часто используемые команды: make k3s-status, make ping, make todo
- Выделена секция "Gotchas" с критичными особенностями
- Сохранены ссылки на docs/ (не дублируется контент)

---

## 2026-03-14 - Обновлён CLAUDE.md: Ubuntu 25.10 НЕ требует cgroup параметров для k3s

**Выяснено:** При диагностике motya (NotReady) оказалось что проблема НЕ в отсутствии `cgroup_memory=1` в cmdline.txt
**Проверка:** На рабочей ноде osya k3s работает БЕЗ этих параметров, ядро 6.17.0-1008-raspi
**Вывод:** Ubuntu 25.10 на Raspberry Pi имеет cgroup v2 включённым по умолчанию

**Обновлено:**
- CLAUDE.md: добавлена секция "K3s на Ubuntu 25.10"
- CLAUDE.md: уточнена структура проекта (inventory.yml в корне)

---

## 2026-03-14 - Добавлена автоматическая синхронизация времени для k3s

**Проблема:** Узел motya стал NotReady после миграции SSD из-за "certificate has expired/not yet valid"
**Причина:** Системное время было в 2025 году, а сертификаты k3s действительны с 2026-03-05
**Решение:** Автоматическая синхронизация времени в роли k3s перед установкой

**Что сделано:**
- Создан `docs/k3s-node-time-sync-problem.md` - подробное описание проблемы и решения
- Обновлена роль k3s: автоматическое включение chrony/systemd-timesyncd и ожидание синхронизации
- Добавлена переменная `k3s_ensure_time_sync: true` (включено по умолчанию)
- Произведено тестирование: сломанное время → синхронизация → k3s восстановился

**Важное замечание:** Raspberry Pi не имеет RTC батареи → время сбрасывается при перезагрузке → NTP должен работать всегда!
